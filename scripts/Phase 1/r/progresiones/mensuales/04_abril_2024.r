...
url <- "https://raw.githubusercontent.com/Joaquin-Barrera-Flores/Football-picks-analytics/main/data/raw/acumulados_mensuales/2024/abril_2024.csv"
...
acumulado$abril <- dmy(acumulado$abril)
...
ajustes_texto <- list(
    ...
    chochos = list(desp_dias = 2, desp_pct = -0.02),
    dani    = list(desp_dias = 1, desp_pct = 0.04),
    velez   = list(desp_dias = 3, desp_pct = -0.03)
)
...
ultimos <- map(jugadores, ~ acumulado %>%
    slice_tail(n = 1) %>%
    select(abril, puntos = all_of(paste0("puntos_", .x))))
...
maximos_ajustados <- map(jugadores, ~ {
    ...
    data.frame(
        ajuste_x = maximo$abril + days(desp$desp_dias),
...
abr_24 <- ggplot(acumulado, aes(x = abril)) +
    map(jugadores, ~ {
        geom_line(
            aes(
    ...
    labs(
        title = "Tabla mensual - Abril",
        caption = "Datos a 30 de abril de 2024",
        x = "Días",
        y = "Puntos",
        color = "Jugador"
    ) +
...
ggsave(
    filename = "outputs/plots/Phase 1/r/progresiones/mensuales/r_tabla_abril_2024.png",
    plot = abr_24,
    width = 16,
    height = 8,
    units = "in",
