...
filtro_mundial_clubes_total <- puntuacion %>%
    filter(
        torneo %in% c(
            "Intercontinental"
        )
...
suma_larga <- filtro_mundial_clubes_total %>%
    pivot_longer(
...
mc <- ggplot(suma_larga, aes(x = torneo, y = Puntos, fill = Jugador)) +
    geom_bar(
        stat = "identity",
    ...
    labs(
        ...
        subtitle = "Comparativa por torneo mundial de clubes",
        x = "Copa",
        y = "Puntos",
        fill = "Jugadores",
...
# Guardar gráfico
ggsave(
    filename = "outputs/plots/r/puntuaciones/comparativas/por_torneo/2024/r_comparativa_puntos_por_mundial_clubes_2024.png",
    plot = mc,
    width = 12,
    height = 10,
    units = "in",
...
write.csv(filtro_mundial_clubes_total, here("data", "processed", "r", "metricas_comparativas", "puntuaciones", "por_torneo", "totales", "2024", "puntuaciones_por_mundial_clubes_2024.csv"))